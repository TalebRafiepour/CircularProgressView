...
class CircularProgressIndicator : View {
    ...
    private var indeterminateStartAngle = startAngle
    private var indeterminateStartProgress = 0.0
    ...
    private var indeterminateDegreeAnimator:ValueAnimator? = null
    private var indeterminateProgressAnimator:ValueAnimator? = null
    ...
    val gradientType: Int
        ...
        get() {
            ...
            when (shader) {
                is LinearGradient -> type = LINEAR_GRADIENT
                is RadialGradient -> type = RADIAL_GRADIENT
                is SweepGradient -> type = SWEEP_GRADIENT
            }
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        if (attrs != null) {
            ...
            val formattingPattern = a.getString(R.styleable.CircularProgressIndicator_formattingPattern)
            ...
            if (gradientType != NO_GRADIENT) {
                val gradientColorEnd = a.getColor(R.styleable.CircularProgressIndicator_gradientEndColor, -1)
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val strokeSizeOffset = if (isDotEnabled) Math.max(dotWidth, Math.max(progressWidth, progressBackgroundWidth)) else Math.max(progressWidth, progressBackgroundWidth)
        ...
        desiredSize += (Math.max(textBoundsRect.width(), textBoundsRect.height()) + desiredSize * .1f).toInt()
    ...
    private fun calculateBounds(w: Int, h: Int) {
        ...
        val progressWidth = progressPaint!!.strokeWidth
        val progressBackgroundWidth = progressBackgroundPaint!!.strokeWidth
        val strokeSizeOffset = if (isDotEnabled) Math.max(dotWidth, Math.max(progressWidth, progressBackgroundWidth)) else Math.max(progressWidth, progressBackgroundWidth) // to prevent progress or dot from drawing over the bounds
    ...
    fun startIndeterminate(durationAnimation: Long = 3000L) {
        indeterminateStartAngle = startAngle
        indeterminateStartProgress = progress

        indeterminateDegreeAnimator = ValueAnimator.ofInt(0, 360).apply {
            addUpdateListener {
        ...
        indeterminateDegreeAnimator?.start()

        indeterminateDegreeAnimator?.addListener(object : DefaultAnimatorListener(){
            override fun onAnimationCancel(animation: Animator) {
                super.onAnimationCancel(animation)
                startAngle = indeterminateStartAngle
                indeterminateDegreeAnimator = null
            }
        })

        setProgress(0.0, maxProgress)
        indeterminateProgressAnimator =  ValueAnimator.ofFloat(0f, (maxProgress*0.85).toFloat()).apply {
            addUpdateListener {
        ...
        indeterminateProgressAnimator?.addListener(object : DefaultAnimatorListener() {
            override fun onAnimationCancel(animation: Animator) {
                super.onAnimationCancel(animation)
                progress = indeterminateStartProgress
                setProgress(progress,maxProgress)
                indeterminateProgressAnimator = null
            }
        })

        indeterminateProgressAnimator?.start()
    }
    fun stopIndeterminate() {
        indeterminateDegreeAnimator?.cancel()
        indeterminateProgressAnimator?.cancel()
    }

    private fun drawProgressBackground(canvas: Canvas) {
        canvas.drawArc(circleBounds!!, ANGLE_START_PROGRESS_BACKGROUND.toFloat(), ANGLE_END_PROGRESS_BACKGROUND.toFloat(),
    ...
    fun setTextSizePx(@Dimension size: Int) {
        ...
        val offset = if (isDotEnabled) Math.max(dotPaint!!.strokeWidth, progressPaint!!.strokeWidth) else progressPaint!!.strokeWidth
    ...
    fun setGradient(@GradientType type: Int, @ColorInt endColor: Int) {
        ...
        when (type) {
            LINEAR_GRADIENT -> gradient = LinearGradient(0f, 0f, width.toFloat(), height.toFloat(), startColor, endColor, Shader.TileMode.CLAMP)
            RADIAL_GRADIENT -> gradient = RadialGradient(cx, cy, cx, startColor, endColor, Shader.TileMode.MIRROR)
            SWEEP_GRADIENT -> gradient = SweepGradient(cx, cy, intArrayOf(startColor, endColor), null)
        }
    ...
    companion object {
        ...
        private val DEFAULT_PROGRESS_START_ANGLE = 270
        private val ANGLE_START_PROGRESS_BACKGROUND = 0
        private val ANGLE_END_PROGRESS_BACKGROUND = 360

        private val DESIRED_WIDTH_DP = 150

        private val DEFAULT_PROGRESS_COLOR = "#3F51B5"
        private val DEFAULT_TEXT_SIZE_SP = 24
        private val DEFAULT_STROKE_WIDTH_DP = 8
        private val DEFAULT_PROGRESS_BACKGROUND_COLOR = "#e0e0e0"

        private val DEFAULT_ANIMATION_DURATION = 1000

        private val PROPERTY_ANGLE = "angle"
    }
}
